...
def play_aviator(message):
    ...
    for _ in range(30):
        time.sleep(0.6)
        multiplier = round(multiplier + random.uniform(0.15, 0.5), 2)
...
def play_dice(message):
    ...
    result = f"🎲 Siz: {user_dice} | 🤖 Bot: {bot_dice}\n"
...
def withdraw(message):
    user_id = message.from_user.id
    if user_balances.get(user_id, 0) < 5000:
        bot.send_message(message.chat.id, "❌ Minimal chiqarish 5000 so‘m.")
        return
    msg = bot.send_message(message.chat.id, "💳 Karta raqamingizni yuboring:")
...
def process_withdraw(message):
    ...
    bot.send_message(ADMIN_ID, f"📤 Yangi chiqarish so‘rovi:\n👤 @{message.from_user.username or user_id}\n💳 Karta: {card}\n💰 Miqdor: {amount} so‘m")
